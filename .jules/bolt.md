...
## 2026-02-12 - File I/O in Hot Paths
**Learning:** Frequent small file reads (like skill definitions) in every agent loop iteration add up significantly. OS cache helps, but Python overhead and repetitive `read_text` still cost time.
**Action:** Cache file content in memory with `mtime` validation for configuration files read frequently.
