...
## 2026-02-13 - [Caching Frequent I/O Reads]
**Learning:** Repetitively reading configuration or markdown files (like `SKILL.md`) from disk causes significant I/O overhead, particularly when performing loops like in `build_skills_summary` or `list_skills`.
**Action:** When working with frequently accessed disk files, implement an in-memory dictionary cache using `path.stat().st_mtime` to track file modifications and avoid unnecessary I/O.
