...
class SkillsLoader:
    ...
    def _read_with_cache(self, path: Path) -> str:
        """Read a file, using a cache based on modification time."""
        try:
            mtime = path.stat().st_mtime
            if path in self._file_cache and self._file_cache[path].get("mtime") == mtime:
    ...
    def get_skill_metadata(self, name: str) -> dict | None:
        ...
        # Check if we already parsed metadata for this content
        for path_data in self._file_cache.values():
            if path_data.get("content") == content:
                if "metadata" in path_data:
