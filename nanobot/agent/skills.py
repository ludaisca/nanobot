...
class SkillsLoader:
    ...
    def load_skill(self, name: str) -> str | None:
        ...
        entry = self._read_skill_file(path)
    ...
    def _read_skill_file(self, path: Path) -> dict | None:
        """Read a skill file with caching."""
        if not path.exists():
            return None

        mtime = path.stat().st_mtime
        if path in self._file_cache and self._file_cache[path]["mtime"] == mtime:
