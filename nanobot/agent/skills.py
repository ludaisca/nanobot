...
class SkillsLoader:
    ...
    def get_skill_metadata(self, name: str) -> dict | None:
        ...
        """
        # Check cache first if we can find the path
        path = None
        workspace_skill = self.workspace_skills / name / "SKILL.md"
        if workspace_skill.exists():
            path = workspace_skill
        ...
        if path:
            ...
            try:
                mtime = path.stat().st_mtime
                cached = self._file_cache.get(str_path)
                if cached and cached["mtime"] == mtime and "metadata" in cached:
                    return cached["metadata"]
            except Exception:
                pass

        # load_skill handles reading and caching the content
        content = self.load_skill(name)
        if not content:
            return None
        ...
        if path:
            ...
            if str_path in self._file_cache:
                self._file_cache[str_path]["metadata"] = metadata
