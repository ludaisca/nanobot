...
class SkillsLoader:
    ...
    def _read_file_cached(self, filepath: Path) -> str | None:
        """Read a file with in-memory caching based on modification time."""
        if not filepath.exists():
            return None

        path_str = str(filepath)
        try:
            # We don't import os here, stat works directly on Path
    ...
    def load_skill(self, name: str) -> str | None:
        ...
        if workspace_skill.exists():
            return self._read_file_cached(workspace_skill)

        # Check built-in
        if self.builtin_skills:
            builtin_skill = self.builtin_skills / name / "SKILL.md"
            if builtin_skill.exists():
                return self._read_file_cached(builtin_skill)
