...
class SkillsLoader:
    ...
    def __init__(self, workspace: Path, builtin_skills_dir: Path | None = None):
        ...
        # ⚡ Bolt: Cache skill file contents (using mtime invalidation) to reduce disk I/O
        self._file_cache = {}

        # ⚡ Bolt: Cache shutil.which results to prevent repeated expensive system calls
        self._which_cache = {}
    ...
    def load_skill(self, name: str) -> str | None:
        ...
        path_to_check = workspace_skill if workspace_skill.exists() else builtin_skill
        if not path_to_check or not path_to_check.exists():
            return None

        # ⚡ Bolt: Check mtime to know if we can return a cached string
        mtime = path_to_check.stat().st_mtime

        if name in self._file_cache:
            cached_mtime, cached_content = self._file_cache[name]
        ...
        # ⚡ Bolt: Store the content + mtime so we skip I/O next time
        if content is not None:
            self._file_cache[name] = (mtime, content)
    ...
    def _get_missing_requirements(self, skill_meta: dict) -> str:
        ...
        for b in requires.get("bins", []):
            # ⚡ Bolt: Check in-memory dict before invoking shutil.which
