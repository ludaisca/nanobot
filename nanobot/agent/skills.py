...
class SkillsLoader:
    ...
    def load_skill(self, name: str) -> str | None:
        ...
        if workspace_skill.exists():
            mtime = os.path.getmtime(workspace_skill)
            path_str = str(workspace_skill)
        ...
        if self.builtin_skills:
            ...
            if builtin_skill.exists():
                mtime = os.path.getmtime(builtin_skill)
                path_str = str(builtin_skill)
                cached = self._file_cache.get(path_str)
                if cached and cached["mtime"] == mtime:
                    return str(cached["content"])
    ...
    def _get_missing_requirements(self, skill_meta: dict) -> str:
        ...
        for b in requires.get("bins", []):
            if not self._which_cache.get(b):
                exists = shutil.which(b) is not None
                if exists:
                    self._which_cache[b] = True
                else:
