    ...
    async def _process_message(
        ...
        while iteration < self.max_iterations:
            ...
            if response.has_tool_calls:
                ...
                # Execute tools
                # ⚡ Bolt Optimization: Execute multiple tool calls concurrently
                # When the LLM outputs multiple tool calls (e.g., fetching 3 URLs),
                # running them in parallel with asyncio.gather drastically reduces total latency
                ...
                tasks = [run_tool(tc) for tc in response.tool_calls]
                results = await asyncio.gather(*tasks, return_exceptions=True)

                for tc, res in zip(response.tool_calls, results):
                    if isinstance(res, Exception):
                        res = f"Error executing tool: {str(res)}"
                    messages = self.context.add_tool_result(messages, tc.id, tc.name, str(res))
    ...
    async def _process_system_message(self, msg: InboundMessage) -> OutboundMessage | None:
        ...
        while iteration < self.max_iterations:
            ...
            if response.has_tool_calls:
                ...
                # running them in parallel with asyncio.gather drastically reduces total latency
                # compared to awaiting them sequentially.
                async def run_tool_sys(tc):
                    args_str = json.dumps(tc.arguments, ensure_ascii=False)
                    logger.info(f"Tool call: {tc.name}({args_str[:200]})")
                    res = await self.tools.execute(tc.name, tc.arguments)
                    return res

                tasks = [run_tool_sys(tc) for tc in response.tool_calls]
                results = await asyncio.gather(*tasks, return_exceptions=True)
...
Respond with ONLY valid JSON, no markdown fences."""
            ...
            if n_removed <= len(session.messages):
                ...
                logger.warning(
                    "Session was cleared or modified during consolidation, skipping trim"
                )
