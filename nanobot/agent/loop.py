    ...
    async def _process_message(self, msg: InboundMessage, session_key: str | None = None) -> OutboundMessage | None:
        ...
        while iteration < self.max_iterations:
            ...
            if response.has_tool_calls:
                ...
                # Execute tools concurrently for improved performance
                # This reduces latency significantly when the LLM returns multiple tool calls (e.g. read multiple files)
                ...
                for tool_id, tool_name, result in tool_results:
                    messages = self.context.add_tool_result(messages, tool_id, tool_name, result)
    ...
    async def _process_system_message(self, msg: InboundMessage) -> OutboundMessage | None:
        ...
        while iteration < self.max_iterations:
            ...
            if response.has_tool_calls:
                ...
                # Execute tools concurrently for system messages
                async def execute_tool_sys(tool_call):
                    args_str = json.dumps(tool_call.arguments, ensure_ascii=False)
                    logger.info(f"Tool call: {tool_call.name}({args_str[:200]})")
                    result = await self.tools.execute(tool_call.name, tool_call.arguments)
                    return tool_call.id, tool_call.name, result

                tool_results = await asyncio.gather(*(execute_tool_sys(tc) for tc in response.tool_calls))
                for tool_id, tool_name, result in tool_results:
                    messages = self.context.add_tool_result(messages, tool_id, tool_name, result)
