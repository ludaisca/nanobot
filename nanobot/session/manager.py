...
class Session:
    ...
    # Internal fields for persistence optimization
...
class SessionManager:
    ...
    def save(self, session: Session) -> None:
        ...
        # conditions:
        # 1. File exists
        # 2. Metadata hasn't changed (since it's in the header)
        # 3. We are only appending messages (persisted count matches prefix)
        can_append = (
            path.exists()
            and session._persisted_count > 0
            and session._persisted_count <= len(session.messages)
            and session.metadata == session._persisted_metadata
        )
        ...
        if can_append:
            ...
            try:
                ...
                logger.error(f"Failed to append to session {session.key}: {e}")
                ...
                try:
                    ...
                    with open(path) as f:
                        ...
                        if first_line:
                            ...
                            if data.get("_type") == "metadata":
                                # Use mtime for updated_at to support append-only updates
                                # where the header might be stale
